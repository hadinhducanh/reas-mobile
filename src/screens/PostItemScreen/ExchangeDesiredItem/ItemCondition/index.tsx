...
const ExchangeDesiredItemConditionScreen = () => {
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const [selectedOption, setSelectedOption] = useState<string | null>(null);

  // Lấy dữ liệu từ AsyncStorage khi vào màn hình
  useEffect(() => {
    const getStoredExchangeDesiredItemCondition = async () => {
      try {
        const storedExchangeDesiredItemCondition = await AsyncStorage.getItem("selectedExchangeDesiredItemCondition");
        if (storedExchangeDesiredItemCondition) {
          setSelectedOption(storedExchangeDesiredItemCondition);
        }
      } catch (error) {
        console.error("Failed to retrieve condition:", error);
      }
    };
    getStoredExchangeDesiredItemCondition();
  }, []);

  // Khi chọn một option, lưu vào AsyncStorage và quay về UploadScreen
  const handleSelectExchangeDesiredItemCondition = async (condition: string) => {
    try {
      await AsyncStorage.setItem("selectedExchangeDesiredItemCondition", condition);
      ...
      <ScrollView contentInsetAdjustmentBehavior="automatic" className="flex-1">
        ...
        {options.map((option, index) => {
          ...
          return (
            <TouchableOpacity
              key={index}
              onPress={() => handleSelectExchangeDesiredItemCondition(option)}
...
export default ExchangeDesiredItemConditionScreen;
